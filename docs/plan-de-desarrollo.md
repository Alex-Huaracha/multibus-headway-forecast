...
- [~] Análisis por franja horaria (pico vs. valle): **descartado** (2026-06-18). Es un *proxy* de volatilidad (las horas pico son justamente las de headways volátiles), y la estratificación por volatilidad ya cuenta esa historia operativa de forma más directa y causal. Además es blindaje (no criterio de éxito) y requiere reconstruir el timestamp por-muestra desde datos que viven en Kaggle (no locales). Rédito marginal bajo. Detalle del blocker: ver memoria `fase-7/timestamp-blocker`.
...
- [~] Robustez frente a días atípicos: **diferido** (2026-06-18). Mismo blocker de timestamp por-muestra que pico/valle (requiere datos de Kaggle). Es blindaje opcional; si un revisor lo pide, se reabre con la reconstrucción local del timestamp (camino determinístico ya mapeado, ver memoria `fase-7/timestamp-blocker`).
- [x] Tablas y figuras candidatas a paper, **congeladas** (2026-06-18): set final + lectura en `docs/resultados/INTERPRETACION.md`. Figuras: `curva-degradacion.png`, `volatilidad-crossover.png`. Tablas: `consolidated_multihorizon.csv`, `significance_multihorizon.csv`, `volatility_multihorizon.csv`. Todas reproducibles desde CSVs versionados (comandos en INTERPRETACION.md).

**Artefacto:** `src/evaluation/{significance,volatility,degradation,metrics}.py` + builders `src/build_{significance_table,degradation_curve,volatility_table,volatility_curve}.py` + CSVs en `docs/resultados/csv-multihorizon/` + figuras `curva-degradacion.png` y `volatilidad-crossover.png` + guía `docs/resultados/INTERPRETACION.md`.
...
**Estado:** Completada (2026-06-18). El núcleo (métricas + curva de degradación + significancia + mecanismo causal vía volatilidad) está cerrado, verificado e interpretado. Los dos criterios técnicos de `objetivo.md` (p<0.05, ≥2 corredores) se cumplen. Pico/valle y días atípicos se descartaron/difirieron por bajo rédito marginal frente al análisis de volatilidad y por fricción de datos (timestamp en Kaggle). Set de figuras/tablas congelado. Listo para Fase 9 (redacción).
