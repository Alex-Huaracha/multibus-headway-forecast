...
- **2 métricas:** MAE[^mae] y RMSE[^rmse], ambas en minutos (cuanto más bajas, mejor). El texto reporta **MAE**, y esa elección es sustantiva: **el cruce de la Sección 3 es específico del MAE**. Bajo RMSE el DL le gana a la persistencia en *todos* los horizontes, incluido h = 1 (9 de 9 celdas corredor×modelo), así que no hay cruce que reportar. La razón es conocida: el RMSE penaliza cuadráticamente los errores grandes, y la persistencia —que a h = 1 acierta casi siempre pero se rompe fuerte en los saltos— es castigada de forma desproporcionada por esa cola. El MAE, que trata todos los minutos de error por igual, es la métrica operativamente correcta acá: para un despachador, equivocarse 2 minutos es el doble de malo que equivocarse 1, no cuatro veces peor. **Todas las afirmaciones de cruce de este documento deben leerse como afirmaciones sobre el MAE**; las cifras de RMSE están en `csv-multihorizon/significance_multihorizon.csv` y en la Figura 1.
...
- El contraste **DL vs XGBoost no recibe un test de significancia por muestra** (Diebold-Mariano / Wilcoxon), como sí lo recibe DL vs persistencia. Los residuos exportados por las dos familias no son emparejables muestra a muestra: el del DL emite una fila por (ventana solapada × bus) —sobrecontando cada objetivo ≈ 4.5×— y no lleva clave por muestra, mientras el del baseline emite una fila por objetivo de test; realinearlos o deduplicarlos exigiría re-entrenar los kernels DL. La significancia de este contraste se reporta, por lo tanto, con un **test de signos a nivel de celda** (8 de 8 en E2+E59, *p* = 0.004; `xgb_vs_lstm_signtest.csv`), inmune a ese sobreconteo pero más grueso que un test por muestra. Se relaciona con la sobreestimación del *n* efectivo por ventanas solapadas (ver más abajo).
...
8. **Sin corrección por comparaciones múltiples.** Se reportan decenas de comparaciones pareadas (entre corredores, horizontes, direcciones y métricas) sin control de familia (Holm / Benjamini-Hochberg). Como la mayoría de los valores-p principales son ≈ 0, la corrección sería cosmética, pero su ausencia debe declararse.
