"""Generate notebooks/11_lstm_multihorizon/11_lstm_h{H}.ipynb for Kaggle.

Produces 4 notebooks — one per horizon h ∈ {1, 3, 5, 10}:
  11_lstm_h1.ipynb   → alexhuaracha/11-lstm-multihorizon-h1
  11_lstm_h3.ipynb   → alexhuaracha/11-lstm-multihorizon-h3
  11_lstm_h5.ipynb   → alexhuaracha/11-lstm-multihorizon-h5
  11_lstm_h10.ipynb  → alexhuaracha/11-lstm-multihorizon-h10
...
Key differences from build_notebook_07.py (the h=1 LSTM notebook):
  ...
  - Train cell reuses WINNING_CONFIGS from docs/resultados/configuraciones-ganadoras.md
    instead of running the full 24-config grid search.
  - Results CSV named lstm_results_h{H}.csv; rows carry a "horizon" column.
...
Inline-embed pattern (mirror of build_notebook_07.py):
  ...
  - Stable cell IDs (cell-11-*) prevent git flutter on re-runs.
...
""",
        cell_id_md="cell-11-embed-splits-md",
        cell_id_code="cell-11-embed-splits",
    )
...
""",
        cell_id_md="cell-11-embed-metrics-md",
        cell_id_code="cell-11-embed-metrics",
    )
...
""",
        cell_id_md="cell-11-embed-windowing-md",
        cell_id_code="cell-11-embed-windowing",
    )
...
""",
        cell_id_md="cell-11-embed-normalization-md",
        cell_id_code="cell-11-embed-normalization",
    )
...
""",
        cell_id_md="cell-11-embed-dataset-md",
        cell_id_code="cell-11-embed-dataset",
    )
...
""",
        cell_id_md="cell-11-embed-lstm-md",
        cell_id_code="cell-11-embed-lstm",
    )
...
`GRID` — 24 TrainConfig entries (kept for reference; NB11 uses WINNING_CONFIGS only).
...
""",
        cell_id_md="cell-11-embed-train-md",
        cell_id_code="cell-11-embed-train",
    )
...
def _add_title_cell(horizon: int) -> None:
    md(
        f"""
# 11 — LSTM Multi-Horizonte h={horizon}  (auto-generado por build_notebook_11.py)

Entrena un modelo LSTM (`HeadwayLSTM`) sobre los corredores **E2** y **E59**
usando ventanas deslizantes con horizonte directo **h={horizon} minuto(s)**.

En vez de hacer un nuevo grid search, se reusa la configuración ganadora de Fase 5
...
HORIZON = {horizon}
LSTM_CSV_OUT = OUTPUT_DIR / f"lstm_results_h{{HORIZON}}.csv"
...
""",
        cell_id="cell-11-dataset",
    )


def _add_train_cell() -> None:
    md(
        """## Entrenamiento LSTM — configuración ganadora por corredor
...
def run_corridor_single(loaders: dict, max_N: int, label: str):
    ...
    print(f"\\n{label} LSTM training: max_N={max_N}, device={DEVICE}")
    results = grid_search(
    ...
    best = results[0]
    print(f"  {label} config: hidden={best.config.hidden_size}, "
...
""",
        cell_id="cell-11-evaluate",
    )


def _add_results_cell() -> None:
    md(
        """## Tabla de resultados — lstm_results_h{H}.csv

Guarda los resultados del LSTM en formato long-form con columna `horizon` adicional
...
# Output file: lstm_results_h{HORIZON}.csv (horizon-discriminated filename)
lstm_results.write_csv(LSTM_CSV_OUT)
print(f"\\nLSTM results written to: {LSTM_CSV_OUT}  ({lstm_results.height} rows)")
print(lstm_results)
""",
        cell_id="cell-11-results",
    )


def _add_residuals_cell() -> None:
    md(
        """## Residuos por muestra — lstm_residuals_h{H}.csv (significancia)
...
""",
        cell_id="cell-11-residuals-md",
    )
    code(
        """
LSTM_RESID_OUT = OUTPUT_DIR / f"lstm_residuals_h{HORIZON}.csv"
...
actual antes de comparar con los resultados del LSTM.
""",
        cell_id="cell-11-compare-md",
    )
    code(
        """
...
def build_horizon_notebook(horizon: int) -> None:
    ...
    out_dir = ROOT / "notebooks" / "11_lstm_multihorizon" / f"h{horizon}"
    out_dir.mkdir(parents=True, exist_ok=True)
    notebook_filename = f"11_lstm_h{horizon}.ipynb"
    ...
    kernel_meta = {
        "id": f"alexhuaracha/11-lstm-multihorizon-h{horizon}",
        "title": f"11 LSTM Multihorizon h{horizon}",
