"""Generate notebooks/17_e4_lstm/h{H}/17_e4_lstm_h{H}.ipynb for Kaggle.

Produces 4 notebooks — one per horizon h ∈ {1, 3, 5, 10}:
  17_e4_lstm_h1.ipynb   → alexhuaracha/17-e4-lstm-h1
  17_e4_lstm_h3.ipynb   → alexhuaracha/17-e4-lstm-h3
  17_e4_lstm_h5.ipynb   → alexhuaracha/17-e4-lstm-h5
  17_e4_lstm_h10.ipynb  → alexhuaracha/17-e4-lstm-h10

E4-ONLY external-validity replication of NB11 (LSTM multi-horizonte). Each
notebook trains the LSTM (`HeadwayLSTM`) on the THIRD corridor **E4** only,
using sliding windows with the DIRECT multi-horizon target alignment (the
single timestep at +h steps after the last input is the target; T_OUT=1).

Key differences from build_notebook_11.py (E2+E59 LSTM notebook):
  ...
  - MINI-GRID instead of a single reused config: E4_MINIGRID holds 3 candidate
    TrainConfig entries (E2-style, E59-style, wider). grid_search returns the
    list sorted ascending by best_val_loss; results[0] IS the mini-grid winner.
  - Output CSVs are E4-scoped: lstm_E4_results_h{H}.csv, lstm_E4_residuals_h{H}.csv.
  - Cell IDs use cell-17-* prefix.

Reuses every library function unchanged via the same inline-embed mechanism as
NB11 (Path.read_text → strip relative imports → code cell).

kernel_sources: ["alexhuaracha/16-e4-data-baselines", "alexhuaracha/02-eda-corridors"]
  (NB16 outputs headways_E4 + the E4 baselines CSV the compare cell reads.)
"""
import json
import sys
...
""",
        cell_id_md="cell-17-embed-splits-md",
        cell_id_code="cell-17-embed-splits",
    )
...
""",
        cell_id_md="cell-17-embed-metrics-md",
        cell_id_code="cell-17-embed-metrics",
    )
...
""",
        cell_id_md="cell-17-embed-windowing-md",
        cell_id_code="cell-17-embed-windowing",
    )
...
""",
        cell_id_md="cell-17-embed-normalization-md",
        cell_id_code="cell-17-embed-normalization",
    )
...
""",
        cell_id_md="cell-17-embed-context-md",
        cell_id_code="cell-17-embed-context",
    )
...
""",
        cell_id_md="cell-17-embed-dataset-md",
        cell_id_code="cell-17-embed-dataset",
    )
...
""",
        cell_id_md="cell-17-embed-lstm-md",
        cell_id_code="cell-17-embed-lstm",
    )
...
""",
        cell_id_md="cell-17-embed-train-md",
        cell_id_code="cell-17-embed-train",
    )
...
def _add_title_cell(horizon: int) -> None:
    md(
        f"""
# 17 — LSTM E4 (validez externa) h={horizon}  (auto-generado por build_notebook_17_e4_lstm.py)
...
def build_horizon_notebook(horizon: int) -> None:
    ...
    out_dir = ROOT / "notebooks" / "17_e4_lstm" / f"h{horizon}"
    out_dir.mkdir(parents=True, exist_ok=True)
    notebook_filename = f"17_e4_lstm_h{horizon}.ipynb"
    ...
    kernel_meta = {
        "id": f"alexhuaracha/17-e4-lstm-h{horizon}",
        "title": f"17 e4 lstm h{horizon}",
