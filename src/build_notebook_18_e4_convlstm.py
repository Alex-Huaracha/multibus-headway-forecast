"""Generate notebooks/18_e4_convlstm/h{H}/18_e4_convlstm_h{H}.ipynb for Kaggle.

Produces 4 notebooks — one per horizon h ∈ {1, 3, 5, 10}:
  18_e4_convlstm_h1.ipynb   → alexhuaracha/18-e4-convlstm-h1
  18_e4_convlstm_h3.ipynb   → alexhuaracha/18-e4-convlstm-h3
  18_e4_convlstm_h5.ipynb   → alexhuaracha/18-e4-convlstm-h5
  18_e4_convlstm_h10.ipynb  → alexhuaracha/18-e4-convlstm-h10

E4-ONLY external-validity replication of NB12 (SpatialConvLSTM multi-horizonte).
Each notebook trains a `SpatialConvLSTM` (Conv1D espacial + LSTM temporal) on the
...
Key differences from build_notebook_12.py (E2+E59 SpatialConvLSTM notebook):
  ...
  - REUSED config: a single TrainConfig (NB12's E2 winning config) — no grid.
  - Output CSVs are E4-scoped: spatial_conv_lstm_E4_results_h{H}.csv,
    spatial_conv_lstm_E4_residuals_h{H}.csv.
  - Cell IDs use cell-18-* prefix.

Reuses every library function unchanged via the same inline-embed mechanism.

kernel_sources: ["alexhuaracha/16-e4-data-baselines", "alexhuaracha/02-eda-corridors"]
"""
import json
import sys
...
""",
        cell_id_md="cell-18-embed-splits-md",
        cell_id_code="cell-18-embed-splits",
    )
...
""",
        cell_id_md="cell-18-embed-metrics-md",
        cell_id_code="cell-18-embed-metrics",
    )
...
""",
        cell_id_md="cell-18-embed-windowing-md",
        cell_id_code="cell-18-embed-windowing",
    )
...
""",
        cell_id_md="cell-18-embed-normalization-md",
        cell_id_code="cell-18-embed-normalization",
    )
...
""",
        cell_id_md="cell-18-embed-context-md",
        cell_id_code="cell-18-embed-context",
    )
...
""",
        cell_id_md="cell-18-embed-dataset-md",
        cell_id_code="cell-18-embed-dataset",
    )
...
""",
        cell_id_md="cell-18-embed-lstm-md",
        cell_id_code="cell-18-embed-lstm",
    )
...
""",
        cell_id_md="cell-18-embed-spatial-md",
        cell_id_code="cell-18-embed-spatial",
    )
...
`SPATIAL_GRID` — 48 TrainConfig entries (kept for reference; NB18 uses one config only).
...
""",
        cell_id_md="cell-18-embed-train-md",
        cell_id_code="cell-18-embed-train",
    )
...
def _add_title_cell(horizon: int) -> None:
    md(
        f"""
# 18 — SpatialConvLSTM E4 (validez externa) h={horizon}  (auto-generado por build_notebook_18_e4_convlstm.py)

Entrena un modelo `SpatialConvLSTM` (Conv1D espacial + LSTM temporal) sobre el
...
Reusa la configuración ganadora del primer corredor (de NB12) como config única
...
HORIZON = {horizon}
SPATIAL_CSV_OUT = OUTPUT_DIR / f"spatial_conv_lstm_E4_results_h{{HORIZON}}.csv"
...
""",
        cell_id="cell-18-dataset",
    )


def _add_train_cell() -> None:
    md(
        """## Entrenamiento SpatialConvLSTM E4 — config reusada

Reusa la configuración ganadora del primer corredor (de NB12) como config única
para E4. Una sola corrida, sin grid. El dispatcher en `train_one_epoch`/`evaluate_epoch`
detecta `model.spatial == True` y pasa `(inp, ctx, input_mask)` en lugar de
`cat([inp, ctx])` (AD-4).
""",
        cell_id="cell-18-train-md",
    )
    code(
        """
# Reused config: NB12 first-corridor winning SpatialConvLSTM config applied to E4.
E4_CONFIG = TrainConfig(conv_channels=1, hidden_size=32, num_layers=1, dropout=0.2, lr=5e-4)
...
""",
        cell_id="cell-18-evaluate",
    )


def _add_results_cell() -> None:
    md(
        """## Tabla de resultados — spatial_conv_lstm_E4_results_h{H}.csv

Guarda los resultados del SpatialConvLSTM en E4 en formato long-form con columna
`horizon` adicional (schema: corridor, direction, baseline, metric, value, horizon).
direction ∈ {"-1", "+1", "aggregate"} — 6 filas para E4.
""",
        cell_id="cell-18-results-md",
    )
    code(
        """
...
spatial_rows = build_spatial_rows("E4", dir_metrics_e4, dir_arrays_e4)

spatial_results = pl.DataFrame(spatial_rows)
# Output file: spatial_conv_lstm_E4_results_h{HORIZON}.csv (corridor + horizon discriminated)
spatial_results.write_csv(SPATIAL_CSV_OUT)
print(f"\\nSpatialConvLSTM E4 results written to: {SPATIAL_CSV_OUT}  ({spatial_results.height} rows)")
print(spatial_results)
""",
        cell_id="cell-18-results",
    )


def _add_residuals_cell() -> None:
    md(
        """## Residuos por muestra — spatial_conv_lstm_E4_residuals_h{H}.csv (significancia)
...
""",
        cell_id="cell-18-residuals-md",
    )
    code(
        """
SPATIAL_RESID_OUT = OUTPUT_DIR / f"spatial_conv_lstm_E4_residuals_h{HORIZON}.csv"
...
actual antes de comparar con los resultados del SpatialConvLSTM en E4.
""",
        cell_id="cell-18-compare-md",
    )
    code(
        """
...
if baselines_csv is not None:
    ...
    print("SpatialConvLSTM E4 results summary:")
...
def build_horizon_notebook(horizon: int) -> None:
    ...
    out_dir = ROOT / "notebooks" / "18_e4_convlstm" / f"h{horizon}"
    out_dir.mkdir(parents=True, exist_ok=True)
    notebook_filename = f"18_e4_convlstm_h{horizon}.ipynb"
    ...
    kernel_meta = {
        "id": f"alexhuaracha/18-e4-convlstm-h{horizon}",
        "title": f"18 e4 convlstm h{horizon}",
