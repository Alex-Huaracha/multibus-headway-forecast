"""Generate notebooks/12_spatial_conv_lstm_multihorizon/12_spatial_conv_lstm_h{H}.ipynb for Kaggle.

Produces 4 notebooks — one per horizon h ∈ {1, 3, 5, 10}:
  12_spatial_conv_lstm_h1.ipynb   → alexhuaracha/12-spatialconvlstm-multihorizon-h1
  12_spatial_conv_lstm_h3.ipynb   → alexhuaracha/12-spatialconvlstm-multihorizon-h3
  12_spatial_conv_lstm_h5.ipynb   → alexhuaracha/12-spatialconvlstm-multihorizon-h5
  12_spatial_conv_lstm_h10.ipynb  → alexhuaracha/12-spatialconvlstm-multihorizon-h10
...
Key differences from build_notebook_11.py (the multi-horizon LSTM notebook):
  - Embeds models/spatial_conv_lstm.py in addition to models/lstm.py.
  - WINNING_CONFIGS include conv_channels field (SpatialConvLSTM-specific).
  - Evaluate cell uses SpatialConvLSTM(...) constructor and model(inp, ctx, input_mask)
    forward (spatial 3-arg dispatch, not torch.cat([inp, ctx])).
  - Results CSV named spatial_conv_lstm_results_h{H}.csv; baseline column = "SpatialConvLSTM".
  - Cell IDs use cell-12-* prefix.

Shared with NB11 (identical logic):
  - HORIZON injection in setup and dataset cells.
  - fast_materialize: window_size = T_IN + HORIZON; target only at t_idx == window_size - 1.
  - make_window_index called with horizon=HORIZON.
...
""",
        cell_id_md="cell-12-embed-splits-md",
        cell_id_code="cell-12-embed-splits",
    )
...
""",
        cell_id_md="cell-12-embed-metrics-md",
        cell_id_code="cell-12-embed-metrics",
    )
...
""",
        cell_id_md="cell-12-embed-windowing-md",
        cell_id_code="cell-12-embed-windowing",
    )
...
""",
        cell_id_md="cell-12-embed-normalization-md",
        cell_id_code="cell-12-embed-normalization",
    )
...
""",
        cell_id_md="cell-12-embed-context-md",
        cell_id_code="cell-12-embed-context",
    )
...
""",
        cell_id_md="cell-12-embed-dataset-md",
        cell_id_code="cell-12-embed-dataset",
    )
...
""",
        cell_id_md="cell-12-embed-lstm-md",
        cell_id_code="cell-12-embed-lstm",
    )
...
""",
        cell_id_md="cell-12-embed-spatial-md",
        cell_id_code="cell-12-embed-spatial",
    )
...
`SPATIAL_GRID` — 48 TrainConfig entries (kept for reference; NB12 uses WINNING_CONFIGS only).
...
""",
        cell_id_md="cell-12-embed-train-md",
        cell_id_code="cell-12-embed-train",
    )
...
def _add_title_cell(horizon: int) -> None:
    md(
        f"""
# 12 — SpatialConvLSTM Multi-Horizonte h={horizon}  (auto-generado por build_notebook_12.py)

Entrena un modelo `SpatialConvLSTM` (Conv1D espacial + LSTM temporal) sobre los
corredores **E2** y **E59** usando ventanas deslizantes con horizonte directo
**h={horizon} minuto(s)**.

En vez de hacer un nuevo grid search, se reusa la configuración ganadora de Fase 6a
para cada corredor. Evalúa MAE/RMSE en test y compara con los baselines multi-horizonte
de NB10.

Referencia: `docs/plan-de-desarrollo.md §6.5 Fase 6.5 — Multi-Horizonte`.
""",
        cell_id="cell-12-title",
    )
...
HORIZON = {horizon}
SPATIAL_CSV_OUT = OUTPUT_DIR / f"spatial_conv_lstm_results_h{{HORIZON}}.csv"
...
""",
        cell_id="cell-12-dataset",
    )


def _add_train_cell() -> None:
    md(
        """## Entrenamiento SpatialConvLSTM — configuración ganadora por corredor

Reusa la configuración ganadora de Fase 6a (grid search original sobre 48 configs).
...
""",
        cell_id="cell-12-train-md",
    )
    code(
        """
WINNING_CONFIGS = {
    "E2":  TrainConfig(conv_channels=1, hidden_size=32, num_layers=1, dropout=0.2, lr=5e-4),
    "E59": TrainConfig(conv_channels=1, hidden_size=32, num_layers=2, dropout=0.2, lr=5e-4),
}

def run_corridor_single(loaders: dict, max_N: int, label: str):
    \"\"\"Train with the winning config for one corridor.\"\"\"
    print(f"\\n{label} SpatialConvLSTM training: max_N={max_N}, device={DEVICE}")
    results = grid_search(
    ...
    best = results[0]
    print(f"  {label} config: conv_channels={best.config.conv_channels}, "
...
""",
        cell_id="cell-12-evaluate-md",
    )
    code(
        """
def evaluate_corridor_model(best_result, test_loaders, max_N, stats, label):
    \"\"\"Evaluate one corridor SpatialConvLSTM model on per-direction test splits.

    Uses SpatialConvLSTM(max_N, conv_channels, ...) constructor.
    Forward call: model(inp, ctx, input_mask) — spatial 3-arg dispatch (AD-1, AD-4).
...
""",
        cell_id="cell-12-evaluate",
    )


def _add_results_cell() -> None:
    md(
        """## Tabla de resultados — spatial_conv_lstm_results_h{H}.csv

Guarda los resultados del SpatialConvLSTM en formato long-form con columna `horizon`
...
def build_spatial_rows(corridor, dir_metrics, dir_arrays):
    ...
    print(f"{corridor} aggregate SpatialConvLSTM: MAE={agg_mae:.4f} min, RMSE={agg_rmse:.4f} min "
...
# Output file: spatial_conv_lstm_results_h{HORIZON}.csv (horizon-discriminated filename)
...
if baselines_csv is not None:
    ...
    comparison = pl.concat([
        ...
        spatial_results.filter(pl.col("metric") == "MAE"),
    ])
...
def build_horizon_notebook(horizon: int) -> None:
    ...
    out_dir = ROOT / "notebooks" / "12_spatial_conv_lstm_multihorizon" / f"h{horizon}"
    out_dir.mkdir(parents=True, exist_ok=True)
    notebook_filename = f"12_spatial_conv_lstm_h{horizon}.ipynb"
    ...
    # h10 keeps the "h10b" replacement slug: the original h10 kernel is
    # corrupt on Kaggle (commit e0757b6) and must never be pushed to again.
    slug_suffix = "h10b" if horizon == 10 else f"h{horizon}"
    kernel_meta = {
        "id": f"alexhuaracha/12-spatialconvlstm-multihorizon-{slug_suffix}",
        "title": f"12 SpatialConvLSTM Multihorizon {slug_suffix}",
