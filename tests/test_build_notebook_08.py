...
Covers:
  AC-NB08-1: python -m src.build_notebook_08 exits 0 and produces
             notebooks/08_spatial_conv_lstm/08_spatial_conv_lstm.ipynb.
  AC-NB08-2: two consecutive runs are byte-identical (idempotency / no cell-ID flutter).
  AC-NB08-3: every cell ID matches 'cell-08-*' (no random UUIDs).
"""
...
NOTEBOOK_08_PATH = ROOT / "notebooks" / "08_spatial_conv_lstm" / "08_spatial_conv_lstm.ipynb"
BUILDER_08 = ROOT / "src" / "build_notebook_08.py"


class TestNotebook08Builder:
    """W3 — Verify build_notebook_08.py produces a correct, stable notebook.

    AC-NB08-1: builder exits 0 and produces notebooks/08_spatial_conv_lstm/08_spatial_conv_lstm.ipynb.
    AC-NB08-2: two consecutive runs leave the working tree clean (byte-identical).
    AC-NB08-3: all cell IDs have the 'cell-08-' prefix (no uuid4 IDs).
    """

    def test_builder_exits_zero_and_writes_notebook(self):
        """AC-NB08-1: python -m src.build_notebook_08 exits 0 and writes the notebook.
        ...
        assert NOTEBOOK_08_PATH.exists(), (
            f"Expected notebook at {NOTEBOOK_08_PATH} but file does not exist"
        )

    def test_cell_ids_cell_08_prefix(self):
        ...
        if not NOTEBOOK_08_PATH.exists():
            subprocess.run(
                [sys.executable, "-m", "src.build_notebook_08"],
                capture_output=True, text=True, cwd=str(ROOT),
                check=True,
        ...
        with open(NOTEBOOK_08_PATH, encoding="utf-8") as f:
        ...
        assert not bad_ids, (
            f"The following cell IDs do not match 'cell-08-*' pattern: {bad_ids}. "
    ...
    def test_second_run_byte_identical(self):
        ...
        bytes_after_first = NOTEBOOK_08_PATH.read_bytes()
        ...
        bytes_after_second = NOTEBOOK_08_PATH.read_bytes()

        assert bytes_after_first == bytes_after_second, (
            "build_notebook_08 is NOT idempotent: two consecutive runs produced "
            "different byte content. Each cell must have an explicit stable ID "
            "like 'cell-08-setup', not a random uuid4."
        )
